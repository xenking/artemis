...
bool SdlRenderer::initialize(PDECODER_PARAMETERS params)
{
    ...
    // Don't create a renderer or pump events for test-only renderers.
    // Test-only renderers might be created on a non-main thread where
    // interaction with the SDL render API is unsafe. On KMSDRM this
    // also prevents SDL_CreateRenderer from recreating the window.
    if (params->testOnly) {
        return true;
    }
